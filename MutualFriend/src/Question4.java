...
public class Question4 {
	...
	public static class FriendMapper extends Mapper<LongWritable, Text, LongWritable, Text>
	{
		...
		/*
		 * The mapper reads the input file , splits and writes in terms of user
		 * id and friends list key value pairs.
		 */
		...
		public void reduce(MaxAgeCustom key, Iterable<Text> values, Context context) throws IOException, InterruptedException 
		{
			for(Text t:values)
			{
				if(output.size()<10)
				{
					...
					context.write(new Text(t.toString().split(",")[0]), new Text((t.toString().split(",")[2]+","+t.toString().split(",")[3]+","+t.toString().split(",")[4]+","+t.toString().split(",")[5])));
				}
			}
		}
	}
